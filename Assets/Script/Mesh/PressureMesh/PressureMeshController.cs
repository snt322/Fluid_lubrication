...
public class PressureMeshController : MonoBehaviour, PressureMesh.ISendMessage
{
    ...
    /// <summary>
    /// 計算結果(圧力)の分布メッシュを表示回数(回/frame)
    /// </summary>
    private int m_UpdatePerFrame = 1;
    ...
    private bool m_IsUpdateDo = false;
    ...
    private void UpdateMesh()
    {
        ...
        for (int num = 0, j = 0; j < z; j++)                    //Z方向
        {
            for (int i = 0; i < x; i++)                         //X方向
            {
                ...
                col = yPressure[num] / max;

                Color color = Color.HSVToRGB(col, 1, 1);                                                      //頂点色
                                                                                                              //                
                colors[num] = color;                                                      //頂点色
            }
        }
    ...
    private void Update()
    {
        if (!m_IsUpdateDo) { return; }

        if(m_LoopCounter > m_UpdatePerFrame)
        {
    ...
    /// メッシュをアップデートする
    /// </summary>
    /// <param name="doUpdate">true：アップデートを実施</param>
    void ISendMessage.DoUpdateMesh(bool doUpdate)
    {
        m_IsUpdateDo = doUpdate;
    }
}
...
namespace PressureMesh
{
    public interface ISendMessage : IEventSystemHandler
    {
        ...
        void DoUpdateMesh(bool doUpdate);
    }
}